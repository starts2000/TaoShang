...
namespace Starts2000.TaoBaoTool
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            Application.Run(Global.Resolve<ApplicationContext>("Operation"));
            Global.Uinit();
        }
        ...
        static bool CheckRun()
        {
            ...
            if(instance == null)
            {
                if(CheckClient())
                {
                    MessageBoxEx.Show("挂机端已运行，操作端与挂机端不能运行在同一计算机上！", 
                        "操作端程序启动", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    Application.Exit();
                    return false;
                }
            }
            ...
            {
                MessageBoxEx.Show("操作端已运行，同一计算机不能同时运行两个操作端程序！", 
                    "操作端程序启动", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        static bool CheckClient()
        {
            Process[] processes = Process.GetProcesses();
            foreach(var process in processes)
            {
                var title = process.MainWindowTitle;
                if(title.Equals(string.Concat(ViewResources.ClientName, "登录")) || 
                    (title.StartsWith(string.Concat(ViewResources.ClientName, " - 会员(")) && 
                    title.EndsWith(")")))
                {
                    return true;
                }
